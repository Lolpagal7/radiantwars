...
import { GlitchPass } from './GlitchPass.jsx'
import { WaterPass } from './WatePass.jsx'
import './styles.css'
...
function Lines({ mouse, count, colors, radius = 2 }) {
    const lines = useMemo(
        () =>
            new Array(count).fill().map(() => {
                ...
                return {
                    ...
                    width: 0.1,
                    speed: Math.max(0.0005, 0.00001 * Math.random()),
                    curve
                }
            }),
        [count]
...
function Swarm({ count, mouse }) {
    ...
    return (
        <>
            <pointLight ref={light} distance={40} intensity={9} color="lightblue" />
    ...
    return (
        <>
            <Canvas
                ...
                <Swarm count={isMobile ? 1000 : 5000} mouse={mouse} />
